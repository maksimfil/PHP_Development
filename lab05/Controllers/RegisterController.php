...
class RegisterController {
    ...
    public function execute() {
        ...
        if (!empty($_POST['email']) && !empty($_POST['password'])) {
            ...
            if (empty($user)) {
                // 4. Create new user
                $pdo->query("INSERT INTO `users` (`email`, `password`, `created_at`) VALUES ('".$_POST['email']."', '". $_POST['password'] ."', '" .date('Y-m-d H:i:s')."')");
                mail($_POST['email'], "Successful registration ", "Ви успішно зареєструвались",
